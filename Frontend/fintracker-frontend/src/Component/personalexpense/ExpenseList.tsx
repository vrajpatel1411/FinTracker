...
const ExpenseList = () =>
{
  ...
  const toggleMenu = useCallback((expenseId: string | undefined) => {
    if (!expenseId) return;
  ...
  const editExpense = useCallback((expense: PersonalExpense) => {
    setIsEdit(true);
    setModal(true);
        ...
        <div className="-m-1.5 overflow-x-auto">
          <div className="p-1.5 min-w-full inline-block align-middle">
            <div className="border border-gray-200 rounded-lg divide-y divide-gray-200">
              <div className="flex flex-wrap py-3 px-4 items-center gap-3">
                <div className="bg-[#121215] relative max-w-3xs border-2 border-gray-200 rounded-lg shadow-sm">
                  ...
                  <input type="text" name="hs-table-search" id="hs-table-search" className="py-1.5 sm:py-2 px-3 ps-9 block w-full sm:w-48 border-gray-200 shadow-2xs rounded-lg sm:text-sm focus:z-10 focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none" placeholder="Search Expense"/>
              ...
              <div className="block sm:hidden divide-y divide-zinc-800">
                ...
                {!isLoading && isError && (
                  <p className="px-4 py-6 text-center text-red-400">{errorMessage}</p>
                )}
                {!isLoading && !isError && expenseList.length === 0 && (
                  <p className="px-4 py-6 text-center text-gray-400">No expenses found.</p>
                )}
                {!isLoading && !isError && expenseList.map((expense) => (
                  <div key={expense.expenseId} className="flex items-center justify-between px-4 py-3 bg-[#17171c] hover:bg-[#1f1f24] transition-colors">
                    {/* Left: info */}
                    <div className="flex flex-col gap-0.5 min-w-0 flex-1">
                      <span className="text-sm font-semibold text-white truncate">{expense.title}</span>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-xs text-zinc-500">{expense.categoryName}</span>
                        <span className="text-xs text-zinc-600">·</span>
                        <span className="text-xs text-zinc-500">{formatDate(expense.expenseDate.replace(/"/g, ''))}</span>
                        {expense.receiptId && (
                          <a href={expense.receiptUrl} className="text-xs text-green-500">receipt</a>
                        )}
                      </div>
                      {expense.description && (
                        <span className="text-xs text-zinc-600 truncate">{expense.description}</span>
                      )}
                    </div>
                    {/* Right: amount + menu */}
                    <div className="flex items-center gap-3 ml-3 shrink-0">
                      <span className="text-sm font-bold text-teal-400">{formatCurrency(expense.amount)}</span>
                      <div className="relative">
                        <MoreHorizIcon
                          ref={iconRef}
                          onClick={() => toggleMenu(expense.expenseId)}
                          className="cursor-pointer text-zinc-400 hover:text-white"
                        />
                        {selectedExpenseId === expense.expenseId && (
                          <Paper ref={menuRef} sx={{ width: 160, backgroundColor: '#2a2a30', position: 'absolute', mt: 1, right: 0, zIndex: 10 }}>
                            <MenuList>
                              <MenuItem sx={{ ':hover': { backgroundColor: '#3d3d45' } }} onClick={() => editExpense(expense)}>
                                <ListItemIcon>✏️</ListItemIcon>
                                <ListItemText>Edit</ListItemText>
                              </MenuItem>
                              <MenuItem sx={{ ':hover': { backgroundColor: '#3d3d45' } }} onClick={() => void deleteExpenseMutation(expense.expenseId ?? '')}>
                                <ListItemIcon>🗑️</ListItemIcon>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  ...
                  {!isLoading && !isError && expenseList.map((expense)=>{
                    ...
                    <td className="px-6 py-3 text-base tracking-wide text-white whitespace-nowrap">{formatDate(expense.expenseDate.replace(/"/g, ''))}</td>
                    ...
                    <td className="px-6 py-3 text-base tracking-wide text-white relative " >
                      ...
                      { 
                        selectedExpenseId===expense.expenseId && (
                        <Paper ref={menuRef} sx={{ width: { xs: '90vw', sm: 320 },maxWidth: '100%',backgroundColor:'#2a2a30',position:'absolute',mt:1,right:4,rounded:2,zIndex:10 }}>
                          <MenuList>
                            <MenuItem sx={{":hover":{
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 px-4 py-3 bg-[#121215] z-0">
